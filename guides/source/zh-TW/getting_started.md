...
本指南介紹了如何啟動和運行Ruby on Rails

閱讀本指南後，你將瞭解：

* 如何安裝 Rails，創建一個新的 Rails 應用程式，並連接你的應用程式到資料庫。
* Rails 的總體佈局
* MVC（模型、視圖、控制器）和 RESTful 設計的基本原则。
* 如何快速產生 Rails 應用程式的起始部分。
...
本指南專為那些希望從零開始創建 Rails 應用程式的初學者而設計。它並不需要你在之前有任何使用 Rails 的經驗。

Rails 是一種使用 Ruby 程式語言的 Web 應用程式框架，如果你之前沒有使用 Ruby 的經驗，那麼直接學習 Rails 會發現學習曲線非常陡峭。這裡有一些 Ruby 的線上學習資源：

* [Ruby 程式語言官方網站](https://www.ruby-lang.org/en/documentation/)
* [免費的程式語言書籍清單](https://github.com/EbookFoundation/free-programming-books/blob/master/books/free-programming-books-langs.md#ruby)

請注意，有些資源雖然仍然非常出色，但涵蓋的 Ruby 版本較舊，可能不包括你在日常 Rails 開發中會看到的一些語法

Rails是什麼？
--------------

Rails 是一個用 Ruby 程式語言編寫的 Web 應用程式開發框架。它的設計初衷是透過假設每個開發人員的基礎需求，讓 Web 應用程式的編寫變得更簡單。與許多其他語言和框架相比，它能讓你用更少的程式碼完成更多的工作。經驗豐富的 Rails 開發人員還表示，它讓 Web 應用程式的開發變得更加有趣。

Rails 是一款有主見的軟體。它會假設有一種 "最好 "的方法來做事情，並鼓勵這種方法，某些情況下甚至會阻止其他方法。如果你學會了 "Rails 模式"，你可能會發現你的工作效率大大提高。如果你堅持將其他語言的舊習慣帶到 Rails 的開發中，並試圖使用在其他地方學到的模式，你可能會有不太愉快的體驗。

Rails 的哲學包含兩個主要的指導原則：

* **不做重複的事：** 不做重複的事(DRY)是軟體開發的一項原則，它指出："在一個系統中，每一項知識都必須有一個單一、明確、具權威的表示"。透過避免重複編寫相同的資訊，我們的程式碼更容易維護、擴充性更強、錯誤也會更少。
* **慣例優於設定：** Rails 對於在 Web 應用程式中做許多事情的最佳方式都已經有自己的看法，並預設採用這套規則，而不是要求您通過無止盡的設定檔來指定所有細節。

建立一個新的 Rails 專案
----------------------------

閱讀本指南的最佳方法是按步驟閱讀，所有步驟都是運行本範例的應用程式所必須的，也不需要其他程式碼或步驟
